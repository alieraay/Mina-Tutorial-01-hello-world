...
console.log('state after init: ', num.toString());
...
try {
  const txn2 = await Mina.transaction(senderAccount, () => {
    zkAppInstance.update(Field(80)); // must be 81 to execute function successfully
  });
  await txn2.prove();
...
console.log('state after fail test: ', num2.toString());
